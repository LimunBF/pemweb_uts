...
<head>
    ...
    <title>Laporan Peminjaman Lab PTIK_{{ date('d-m-Y') }}</title>
...
<body>
    ...
    <div class="no-print fixed top-0 left-0 w-full bg-white/90 backdrop-blur-sm border-b border-gray-200 p-3 flex justify-between items-center shadow-sm z-50">
        <div class="flex items-center gap-2">
            ...
            <div>
                <h1 class="font-bold text-gray-800 text-sm leading-tight">Pratinjau Cetak</h1>
                <p class="text-[10px] text-gray-500">Ukuran A4 - Margin 1.5cm</p>
            </div>
        </div>
        <div class="flex gap-2">
            <button onclick="window.history.back()" class="px-3 py-1.5 bg-gray-100 border border-gray-300 text-gray-700 rounded text-xs hover:bg-gray-200 transition">
                &larr; Kembali
            </button>
    ...
    <div class="paper">
        ...
        <div class="border-b-2 border-lab-pink pb-3 mb-5 flex items-center justify-between">
            ...
            <div class="text-right">
                <h2 class="text-lg font-bold text-lab-pink uppercase tracking-wide">Laporan Peminjaman</h2>
        ...
        <div class="mb-5 bg-gray-50 rounded border border-gray-100 p-3 flex justify-between items-center text-xs">
            <div>
                <span class="block text-gray-400 uppercase font-bold text-[10px]">Periode</span>
            ...
            <div>
                <span class="block text-gray-400 uppercase font-bold text-[10px]">Kategori User</span>
                <span class="font-bold text-gray-800">{{ request('role') ? ucfirst(request('role')) : 'Semua User' }}</span>
            </div>
            <div>
                <span class="block text-gray-400 uppercase font-bold text-[10px]">Status</span>
                <span class="font-bold text-gray-800">{{ request('status') ? ucwords(str_replace('_', ' ', request('status'))) : 'Semua Status' }}</span>
            </div>
            <div class="text-right">
                <span class="block text-gray-400 uppercase font-bold text-[10px]">Total Data</span>
                <span class="font-bold text-lab-pink text-sm">{{ $peminjaman->count() }}</span>
            </div>
        </div>
        ...
        <table class="w-full text-xs text-left border border-gray-300 mb-8">
            <thead>
                <tr class="table-header text-white uppercase text-[10px] tracking-wider">
                    ...
                    <th class="p-2 border border-gray-300 w-1/4">Peminjam</th>
                    <th class="p-2 border border-gray-300 w-1/4">Barang</th>
                    <th class="p-2 border border-gray-300 text-center">Tgl Pinjam</th>
                    <th class="p-2 border border-gray-300 text-center">Tgl Kembali</th>
            ...
            <tbody>
                ...
                <tr class="{{ $index % 2 == 0 ? 'bg-white' : 'bg-gray-50' }}">
                    ...
                    <td class="p-2 border border-gray-300">
                        <div class="font-bold text-gray-800">{{ $pinjam->user->name ?? '-' }}</div>
                        <div class="text-[10px] text-gray-500">{{ $pinjam->user->identity_number ?? '' }} <span class="text-gray-300 mx-1">|</span> {{ ucfirst($pinjam->user->role ?? '') }}</div>
                    </td>
                    
                    <td class="p-2 border border-gray-300">
                        <div class="font-medium text-gray-800">{{ $pinjam->item->nama_alat ?? '-' }}</div>
                        <div class="text-[10px] font-mono text-gray-500">{{ $pinjam->item->kode_alat ?? '' }}</div>
                    </td>
                    
                    <td class="p-2 border border-gray-300 text-center font-mono text-[10px]">
                    ...
                    <td class="p-2 border border-gray-300 text-center">
                         <span class="font-bold text-[10px] uppercase
                            @if($pinjam->status == 'menunggu_persetujuan' || $pinjam->status == 'pending') text-yellow-600
                            @elseif($pinjam->status == 'disetujui') text-green-600
                            @elseif($pinjam->status == 'ditolak') text-red-600
                ...
                @empty
                <tr>
                    <td colspan="6" class="p-6 text-center text-gray-400 italic border border-gray-300 bg-gray-50">
