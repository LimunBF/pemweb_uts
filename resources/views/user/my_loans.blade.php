...
<div class="container mx-auto">
    ...
    {{-- Pisahkan data Active dan History di View ini --}}
    @php
        $activeLoans = $loans->filter(fn($l) => in_array($l->status, ['pending', 'disetujui', 'terlambat']));
        $historyLoans = $loans->filter(fn($l) => in_array($l->status, ['dikembalikan', 'ditolak']));
    @endphp
    ...
    <div class="mb-10">
        <div class="flex items-center mb-6">
            <div class="bg-lab-pink-btn p-2 rounded-lg mr-3 shadow-sm text-white">
        ...
        <div class="grid grid-cols-1 gap-5">
            @forelse($activeLoans as $loan)
                <div class="bg-white rounded-xl shadow-md border-l-8 p-6 flex flex-col md:flex-row justify-between items-center hover:shadow-lg transition-shadow duration-200 
                    {{ $loan->status == 'terlambat' ? 'border-red-500' : ($loan->status == 'pending' ? 'border-yellow-400' : 'border-blue-500') }}">
                    ...
                    <div class="flex items-center w-full md:w-1/3 mb-4 md:mb-0">
                        ...
                        <div>
                            <h3 class="text-lg font-bold text-gray-800">{{ $loan->item->nama_alat }}</h3>
                    ...
                    {{-- Tengah: Info Tanggal (Layout Grid biar Rapi) --}}
                    <div class="grid grid-cols-2 gap-8 w-full md:w-1/3 text-center md:text-left mb-4 md:mb-0 border-t md:border-t-0 md:border-l border-b md:border-b-0 md:border-r border-gray-100 py-3 md:py-0 md:px-6">
                        ...
                        <div>
                            <span class="block text-[10px] uppercase tracking-wider font-bold {{ $loan->status == 'terlambat' ? 'text-red-500' : 'text-blue-500' }}">
                                Deadline
                            </span>
                            <span class="text-lg font-bold {{ $loan->status == 'terlambat' ? 'text-red-600' : 'text-gray-800' }}">
                                {{ \Carbon\Carbon::parse($loan->tanggal_kembali)->format('d M Y') }}
                            </span>
                            @if($loan->status == 'terlambat')
                                <span class="block text-[10px] text-red-500 font-bold">Telat {{ \Carbon\Carbon::now()->diffInDays(\Carbon\Carbon::parse($loan->tanggal_kembali)) }} hari!</span>
                            @else
                                <span class="block text-[10px] text-gray-400">{{ \Carbon\Carbon::parse($loan->tanggal_kembali)->diffForHumans() }}</span>
                            @endif
                        </div>
                    </div>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="font-medium text-gray-700">{{ $loan->item->nama_alat }}</span>
                        </td>
